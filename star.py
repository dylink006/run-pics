...
def calculate_star_score(G, route_nodes, star_template_coords, size):
    ...
    try:
        ...
        star_poly = Polygon(star_template_coords).buffer(0)
        route_line = LineString(route_coords)
        star_line = LineString(star_template_coords)

        # IoU
        if route_poly.is_valid and star_poly.is_valid:
            inter_area = route_poly.intersection(star_poly).area
            union_area = route_poly.union(star_poly).area
        ...
        # Hausdorff distance
        h_dist = max(
            directed_hausdorff(np.array(route_coords), np.array(star_template_coords))[0],
            directed_hausdorff(np.array(star_template_coords), np.array(route_coords))[0]
        )
...
def build_and_run_star_matrix(city="San Francisco, California"):
    ...
    sizes = [0.005, 0.01, 0.015, 0.02]  # Different star sizes to try
    results = []
    ...
    for north_offset in latitude_offsets:
        for east_offset in longitude_offsets:
            for size in sizes:
                ...
                # Score the route
                ideal_star_coords = [(lon, lat) for lat, lon in star_points]

                score = calculate_star_score(G, route_nodes, ideal_star_coords, size)
